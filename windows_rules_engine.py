...
def generate_windows_recommendations(alerts):
    ...
    if "user_security" in categories:
        ...
        actions = [
            "立即检查所有管理员账户的合法性，重点关注非内置Administrator账户",
            "审查用户权限分配，确保遵循最小权限原则"
        ]
        
        if high_privilege_count > 0:
            actions.append(f"发现 {high_privilege_count} 个超级管理员权限用户，建议与运维开发人员确认其业务必要性")
        
        if admin_count > 0:
            actions.append("管理员账户数量异常，建议：1) 禁用不必要的管理员账户 2) 将部分账户降级为普通用户")
        ...
        recommendations.append({
            "category": "用户安全",
            "priority": "高",
            "risk_level": "中等" if admin_count == 0 else "高",
...
if __name__ == '__main__':
    # 生产环境配置
    port = int(os.environ.get('PORT', 12002))  # 使用不同的端口避免冲突
