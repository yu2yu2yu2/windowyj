...
function Get-SecurityStatsSummary {
    ...
    if ($riskScore -le 20) {
        $output += "✓ 系统安全状况良好，建议继续保持当前安全配置`n"
        $output += "✓ 定期进行安全检查和更新`n"
    } elseif ($riskScore -le 50) {
        $output += "⚠ 系统存在中等安全风险，建议：`n"
        $output += "  1. 审查管理员权限分配`n"
        $output += "  2. 检查可疑进程和网络连接`n"
        $output += "  3. 启用所有安全防护功能`n"
    } else {
        $output += "🚨 系统存在较高安全风险，建议立即：`n"
        $output += "  1. 隔离可疑进程和网络连接`n"
        $output += "  2. 全面检查用户权限`n"
        $output += "  3. 启用所有安全防护并进行全盘扫描`n"
        $output += "  4. 联系安全专家进行深度分析`n"
    }
